...
export function piece<T>(board: Board<T>, p: Position): T | undefined {
    if (isPositionOutsideOfGrid(board, p))
        return undefined;
    return board.grid[p.row][p.col];
}

export function canMove<T>(board: Board<T>, first: Position, second: Position): boolean {
    if (isPositionOutsideOfGrid(board, first) || isPositionOutsideOfGrid(board, second))
        return false
...
function isPositionOutsideOfGrid<T>(board: Board<T>, p: Position) {
    return (p.col < 0 || p.col >= board.width) || (p.row < 0 || p.row >= board.height);
}
...
function checkAndHandleMatch<T>(board: Board<T>, generator: Generator<T>, moveResult: MoveResult<T>): MoveResult<T>  {
