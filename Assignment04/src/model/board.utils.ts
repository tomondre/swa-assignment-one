import type { Board, Generator, Position, MoveResult, Match } from './board';
...
function removePositions<T>(grid: (T | null)[][], positions: Position[]) {
  ...
  for (let h = board.height - 1; h >= 0; h--) {
    for (let w = 0; w < board.width; w++) {
      if (!piece(board, { row: h, col: w }))
        board.grid[h][w] = takeNumberAbove(board, { row: h, col: w }) as T;
    }
  }
...
function takeNumberAbove<T>(board: Board<T>, position: Position): T | null {
  for (let r = position.row - 1; r >= 0; r--) {
    ...
    if (value) {
      board.grid[r][position.col] = null as T;
      return value;
    }
  }
  return null;
